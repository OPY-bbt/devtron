  ...
  return Eval.execute(() => {
    ...
    ipcRenderer.invoke = function(channel) {
      ...
      return new Promise(async (resolve, reject) => {
        try {
          trackEvent(channel, Array.prototype.slice.call(args, 1), true, true)
          const returnValue = await originalInvoke.apply(ipcRenderer, arguments)
          trackEvent(channel, [returnValue], false, false)
          resolve(returnValue)
        } catch(err) {
          reject(err)
        }
